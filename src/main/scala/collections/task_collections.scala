...
object task_collections {
  ...
  def numbersToNumericString(text: String): String = {
    val map = Map('0' -> "zero", '1' -> "one", '2' -> "two", '3' -> "three", '4' -> "four", '9' -> "nine")

    text.toList.flatMap(x => {
      map.get(x) match {
  ...
  def intersectionAuto(dealerOne: Iterable[Auto], dealerTwo: Iterable[Auto]): Iterable[Auto] = {
    val autos: Iterable[(Auto, Auto)] = for {
      one <- dealerOne
      two <- dealerTwo
    } yield {(one, two)}

    autos.flatMap(pair => List(pair._1, pair._2)).toSet
  }
  ...
  def filterAllLeftDealerAutoWithoutRight(dealerOne: Iterable[Auto], dealerTwo: Iterable[Auto]): Iterable[Auto] = {
    val autos = for {
      one <- dealerOne if !dealerTwo.exists(x => x == one)
    } yield one
    autos.toSet
  }
}